...
def iter_images(chapter_dir: Path) -> list[Path]:
    return sorted(
...
def page_dimensions(images: list[Path]) -> tuple[int, int]:
    ...
    if width <= 0 or height <= 0:
        raise ValueError("unable to determine page dimensions")
    return width, height
...
def nav_document(title: str, page_count: int, language: str) -> str:
    ...
    lines = [
        ...
        '  <nav epub:type="toc">',
        "    <ol>",
        '      <li><a href="page000.xhtml">开始阅读</a></li>',
        "    </ol>",
        "  </nav>",
    ...
    for index in range(page_count):
        lines.append(f'      <li><a href="page{index:03d}.xhtml">{index + 1}</a></li>')
...
<body>
  <img src="images/{index:03d}.jpg" alt="第{index + 1}页"/>
</body>
</html>"""
...
def create_epub(
    ...
    *,
    ...
    images = iter_images(chapter_dir)
    if not images:
        print(f"no images found in {chapter_dir}", file=sys.stderr)
        return False

    page_width, page_height = page_dimensions(images)
    safe_title = escape(title)
  ...
  <rootfiles>
    <rootfile full-path="OEBPS/content.opf" media-type="application/oebps-package+xml"/>
        ...
        for index, _ in enumerate(images):
            ...
            opf_lines.append(
                f'    <item id="img{index}" href="images/{index:03d}.jpg" media-type="image/jpeg"{image_properties}/>'
            )
            opf_lines.append(
        ...
        for index, image_path in enumerate(images):
            epub.writestr(
                f"OEBPS/images/{index:03d}.jpg",
                rendered_image_bytes(image_path, (page_width, page_height), background, jpeg_quality),
            )
            epub.writestr(f"OEBPS/page{index:03d}.xhtml", page_document(title, index, page_width, page_height, language))
    return True


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Create a fixed-layout chapter EPUB from a folder of images")
...
def main(argv: list[str]) -> int:
    ...
    return 0 if create_epub(
        Path(args.chapter_dir).resolve(),
